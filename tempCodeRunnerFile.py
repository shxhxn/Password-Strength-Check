...
# For a self-contained app, we use a small, representative list of common words and breached passwords.
COMMON_WEAK_WORDS = [
    "password", "123456", "qwerty", "admin", "qazwsx", "football", "summer", 
    "spring", "welcome", "secure", "secret", "test", "shadow", "master", 
    "default", "change", "dropbox", "america", "india", "ganesh", "shahan" # Added name examples
]

# 2. Keyboard Patterns (V2)
KEYBOARD_PATTERNS = [
...
def calculate_modified_score(password, entropy_bits):
    """
    Combines Entropy with deductions for known weaknesses (V1, V2, V3, V7).
    ...
    if repeat_match:
        score -= len(repeat_match) * 10 # Adjusted to fit new scoring scale
    ...
    # Simple heuristic: Name or common word + Year + Symbol (e.g. India@123, Shahan2005!)
...
def get_strength_details(password):
    ...
    # We use industry-standard entropy levels:
    # < 35 bits: Too Weak/Weak (seconds to hours)
    # 35-50 bits: Moderate (days to years)
    # 51-65 bits: Strong (decades to centuries)
    # > 65 bits: Excellent (millennia)
    ...
    for word in COMMON_WEAK_WORDS:
        if word in password.lower():
            ...
            is_weak_word = True
            break
    if not is_weak_word and effective_entropy < initial_entropy:
        # Generic warning for pattern deductions if no specific word was found
        feedback.append("⚠️ Warning: Contains easily guessable patterns or predictable components (V2/V7).")
...
class PasswordCheckerApp:
    def __init__(self, master):
        ...
        # Score (Entropy Bits)
        self.score_label = ttk.Label(self.main_frame, text="Entropy: 0 bits", font=('Segoe UI', 9), foreground='#888')
        self.score_label.pack(anchor='e')

        # Feedback Section
        ttk.Label(self.main_frame, text="Security Checklist:", style='Bold.TLabel').pack(anchor='w', pady=(10, 5))
    ...
    def check_strength_event(self, event):
        ...
        self.score_label.config(text=f"Entropy: {details['score']} bits")
